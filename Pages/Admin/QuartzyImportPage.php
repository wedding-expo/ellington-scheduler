...
class QuartzyImportPage extends ActionPage
{
	...
	{
		$this->Display('Admin/quartzy_import.tpl');
	}
...
class QuartzyImportPresenter extends ActionPresenter
{
	...
	public function Import()
	{
		...
		{
			$extractDirectory = str_replace('.tmp', '', $file->TemporaryName());
	...
	private function AddUser($email)
	{
		...
		if (empty($userId))
		{
			...
			$password = $enc->EncryptPassword(Password::GenerateRandom());
			$userId = $this->userRepository->Add(User::Create('', '', $email, $email, 'en_us',
