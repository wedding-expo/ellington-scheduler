...
<div id="help">
	...
	<p>If you are in a group with the Application Administrator role assigned then you will see the Application Management menu item. All
	...
	<div id="help-schedules">
		...
		<p>Schedules can be displayed starting on any day of the week and for any number of days. For a schedule to display starting on the current day, set the
		...
		<p>A group of users may be set up with permission to manage resources. In order for a group to be set as the schedule administrator the group must first
			be granted the Schedule Administrator role. This is configured from the Groups admin tool. Once that role has been added, the group will be
			available in the Manage Schedules tool.</p>

		<p>Schedule Administrators have the same capabilities as Application Administrators for any resource that is on a schedule which the group is assigned
	...
	<div id="help-resources">
		...
		<p>You can prevent resources from being booked too far into the future by requiring a maximum notification of
			...
			in the future, the resource will not be able to be booked past 10:30 AM on Tuesday. The default is no maximum.</p>
		...
		<p>Application Administrators and applicable Schedule and Resource Administrators are exempt from usage constraints.</p>
	...
	<div id="help-configuration">
		...
		<p class="setting"><span>$conf['settings']['app.title']</span>The title of the application to be used in the browser. Default is false.</p>
		...
		{literal}
			<p class="setting"><span>$conf['settings']['reservation.labels']['reservation.popup']</span>The format of what to display in reservation popups.
				Possible values are {name} {dates} {title} {resources} {participants} {accessories} {description} {attributes}. Custom attributes can be
	...
	<p>Next, open Application Management - Customization - Application Configuration and choose the Authentication-ActiveDirectory file.</p>

	<p class="setting"><span>$conf['settings']['domain.controllers']</span>Comma separated list of ActiveDirectory servers. ex) domaincontroller1,controller2
	</p>

	<p class="setting"><span>$conf['settings']['port']</span>Default port 389 or 636 for SSL.</p>

	<p class="setting"><span>$conf['settings']['username']</span>Admin user to bind with (this is not always required).</p>

	<p class="setting"><span>$conf['settings']['password']</span>Admin user password to bind with (this is not always required).</p>

	<p class="setting"><span>$conf['settings']['basedn']</span>The base dn for your domain. This is generally the same as your account suffix, but broken up and
		prefixed with DC=. Your base dn can be located in the extended attributes in Active Directory Users and Computers MMC.</p>

	<p class="setting"><span>$conf['settings']['version']</span>LDAP protocol version. Default is 3</p>

	<p class="setting"><span>$conf['settings']['use.ssl']</span>Whether or not to use SSL. This typically relates to the port used.</p>

	<p class="setting"><span>$conf['settings']['account.suffix']</span>The full account suffix for your domain. Example: @uidauthent.domain.com.</p>

	<p class="setting"><span>$conf['settings']['database.auth.when.ldap.user.not.found']</span>If Active Directory auth fails, authenticate against Booked
		Scheduler database</p>

	<p class="setting"><span>$conf['settings']['attribute.mapping']</span>Mapping of required attributes to attribute names in your directory.</p>

	<p class="setting"><span>$conf['settings']['required.groups']</span>A required group that the user must belong to. The user only needs to belong to at least
		one listed. Blank for no restriction. ex) Group1,Group2</p>

	<p class="setting"><span>$conf['settings']['sync.groups']</span>Whether or not to sync group membership into Booked. The Active Directory groups must first
		be created in Booked. Anything that doesn't exist in Booked will be skipped.</p>

	<p>More info for ActiveDirectory configuration can be found at <a href="http://adldap.sourceforge.net/wiki/doku.php?id=documentation_configuration">http://adldap.sourceforge.net/wiki/doku.php?id=documentation_configuration</a>
	</p>
	...
	<p>Next, open Application Management - Customization - Application Configuration and choose the Authentication-Ldap file.</p>

	<p class="setting"><span>$conf['settings']['host']</span>Comma separated list of LDAP servers such as mydomain1,localhost</p>
	...
	<p class="setting"><span>$conf['settings']['binddn']</span>The distinguished name to bind as (username). If you don't supply this, an anonymous bind will be
		established.</p>

	<p class="setting"><span>$conf['settings']['bindpw']</span>Password for the binddn. If the credentials are wrong, the bind will fail server-side and an
	...
	<p class="setting"><span>$conf['settings']['required.group']</span>A required group that the user must belong to. The user only needs to belong to at least
		one listed. Blank for no restriction. ex) Group1,Group2</p>
	...
	<p class="setting"><span>$conf['settings']['user.id.attribute']</span>The attribute name for user identification. ex) uid</p>

	<p>More info for LDAP configuration can be found at <a href="http://pear.php.net/manual/en/package.networking.net-ldap2.connecting.php">http://pear.php.net/manual/en/package.networking.net-ldap2.connecting.php</a>
	</p>
	...
	<p>Next, open Application Management - Customization - Application Configuration and choose the Authentication-CAS file.</p>
	...
	<p class="setting"><span>$conf['settings']['cas.change.session.id']</span>Whether or not to allow phpCAS to change the session_id.</p>

	<p class="setting"><span>$conf['settings']['email.suffix']</span>Email suffix to use when storing CAS user account. ex) Email addresses will be saved to
		Booked Scheduler as username@yourdomain.com</p>

	<p class="setting"><span>$conf['settings']['cas_logout_servers']</span>Comma separated list of servers to use for logout. Leave blank to not use cas logout
		servers.</p>

	<p class="setting"><span>$conf['settings']['cas.certificates']</span>Full path to certificate to use for CAS. Leave blank if no certificate should be used.
	</p>

	<p class="setting"><span>$conf['settings']['cas.debug.enabled']</span>If detailed CAS logs should be enabled.</p>

	<p class="setting"><span>$conf['settings']['cas.debug.file']</span>Full path to debug file if cas.debug.enabled is true.</p>

	<p>When using CAS you should also set <span class="setting">$conf['settings']['logout.url']</span> to your CAS logout server</p>

	<p>More info for CAS configuration can be found at <a href="https://wiki.jasig.org/display/casc/phpcas">https://wiki.jasig.org/display/casc/phpcas</a></p>

	<h2>WordPress Integration</h2>

	<p>Booked can authenticate your users against a WordPress site running on the same server as Booked. To enable this, first set <span class="setting">$conf['settings']['plugins']['Authentication'] = 'WordPress';</span>
	</p>

	<p>Next, open Application Management - Customization - Application Configuration and choose the Authentication-WordPress file.</p>

	<p class="setting"><span>$conf['settings']['wp_includes.directory']</span>The full path to your wp-includes directory or path relative to Booked Scheduler
		root.</p>

	<p class="setting"><span>$conf['settings']['database.auth.when.wp.user.not.found']</span>If WordPress auth fails, authenticate against Booked Scheduler
		database.</p>
